...
/**
 * 创建日期:2015-12-18 作者： 陈玉婵 修改日期:2016-01-05 作者： 李怡萱 备注：增加司机分布按钮用例
 */
public class SwichWorkTypeCase extends UiAutomatorTestCase {
	...
	public void kiloSetCase() {
		main.kiloSetMAXText();
		main.workBtnClick();
		do{
			sleep(1000);
		}while(base.loadingImgExist());	
		if (main.errInfoKiloExist()) {
			assertEquals("输入行驶里程数过大，请检查", main.errInfoKiloText());
			main.kiloTextClear();
			main.kiloSetText();
			main.workBtnClick();
			do{
